...
use crate::{
    schema::state_value_by_key_hash::StateValueByKeyHashSchema, state_kv_db::StateKvDb, Libra2DB,
};
...
pub fn validate_db_data(
    ...
    let libra2_db = Libra2DB::new_for_test_with_sharding(db_root_path, 1000000);
    let batch_size = 20_000;
    let start_version = libra2_db.get_first_txn_version()?.unwrap();
    target_ledger_version = std::cmp::min(
        libra2_db.get_synced_version()?.unwrap(),
