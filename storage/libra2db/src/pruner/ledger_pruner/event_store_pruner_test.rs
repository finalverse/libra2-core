...
use crate::{Libra2DB, EventStore, LedgerPrunerManager, PrunerManager};
...
fn verify_event_store_pruner(events: Vec<Vec<ContractEvent>>) {
    ...
    let event_store = &libra2_db.event_store;
    ...
    libra2_db.ledger_db.event_db().write_schemas(batch).unwrap();
