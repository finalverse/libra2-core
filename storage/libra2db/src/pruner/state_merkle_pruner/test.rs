...
fn test_state_store_pruner() {
    ...
    let libra2_db = Libra2DB::new_for_test_no_cache(&tmp_dir);
    let state_store = &libra2_db.state_store;
    ...
    {
        let pruner =
            create_state_merkle_pruner_manager(&libra2_db.state_merkle_db(), prune_batch_size);
...
fn test_state_store_pruner_partial_version() {
    ...
    let tmp_dir = TempPath::new();
    let libra2_db = Libra2DB::new_for_test_with_sharding(&tmp_dir, 0);
    let state_store = &libra2_db.state_store;
    ...
    if libra2_db.state_merkle_db().sharding_enabled() {
...
fn verify_state_value_pruner(inputs: Vec<Vec<(StateKey, Option<StateValue>)>>) {
    let tmp_dir = TempPath::new();
    let db = Libra2DB::new_for_test(&tmp_dir);
