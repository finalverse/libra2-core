...
/// This is a wrapper around [Libra2DB] that is used to bootstrap the node for fast sync mode
pub struct FastSyncStorageWrapper {
    // Used for storing genesis data during fast sync
    temporary_db_with_genesis: Arc<Libra2DB>,
    // Used for restoring fast sync snapshot and all the read/writes afterwards
    db_for_fast_sync: Arc<Libra2DB>,
...
impl FastSyncStorageWrapper {
    ...
    /// Otherwise, we return Libra2DB directly and the FastSyncStorageWrapper is None
    pub fn initialize_dbs(
        config: &NodeConfig,
        internal_indexer_db: Option<InternalIndexerDB>,
        update_sender: Option<Sender<(Instant, Version)>>,
    ) -> Result<Either<Libra2DB, Self>> {
        let mut db_main = Libra2DB::open(
                ...
                == 0)
        {
            ...
            let secondary_db = Libra2DB::open(
    ...
    pub fn get_fast_sync_db(&self) -> Arc<Libra2DB> {
        self.db_for_fast_sync.clone()
    }

    pub fn get_temporary_db_with_genesis(&self) -> Arc<Libra2DB> {
