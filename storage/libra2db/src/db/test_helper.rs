...
fn verify_epochs(db: &Libra2DB, ledger_infos_with_sigs: &[LedgerInfoWithSignatures]) {
    const LIMIT: usize = 2;
...
pub fn test_save_blocks_impl(
    ...
    let db =
        Libra2DB::new_for_test_with_buffered_state_target_items(&tmp_dir, snapshot_size_threshold);
