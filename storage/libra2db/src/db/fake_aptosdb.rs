...
/// Alternate implementation of [Libra2DB] for use with consensus-only-perf-test feature.
/// It stores and serves data from in-memory data structures as opposed to [Libra2DB],
/// which uses RocksDB. Note that FakeLibra2DB re-implements only a subset of the
/// features of [Libra2DB] while passing through remaining features to the wrapped inner
/// [Libra2DB].
pub struct FakeLibra2DB {
    inner: Libra2DB,
...
impl FakeLibra2DB {
    ...
    fn save_and_compute_root_hash(
        ...
        let (root_hash, writes) =
            MerkleAccumulator::<FakeLibra2DB, TransactionAccumulatorHasher>::append(
                self,
    ...
    fn get_frozen_subtree_hashes(&self, num_transactions: u64) -> Result<Vec<HashValue>> {
        MerkleAccumulator::<FakeLibra2DB, TransactionAccumulatorHasher>::get_frozen_subtree_hashes(
            self,
            num_transactions,
        )
        .map_err(Into::into)
...
impl DbWriter for FakeLibra2DB {
...
impl DbReader for FakeLibra2DB {
    ...
    fn get_pre_committed_ledger_summary(&self) -> Result<LedgerSummary> {
        // If the genesis is not executed yet, we need to get the executed trees from the inner Libra2DB
        // This is because when we call save_transactions for the genesis block, we call [Libra2DB::save_transactions]
        // where there is an expectation that the root of the SMTs are the same pointers. Here,
        // we get from the inner Libra2DB which ensures that the pointers match when save_transactions is called.
...
/// requests.
impl HashReader for FakeLibra2DB {
...
mod tests {
    ...
    proptest! {
        ...
        fn test_save_transactions(input in arb_blocks_to_commit()) {
            ...
            let db = FakeLibra2DB::new(Libra2DB::new_for_test(&tmp_dir));
