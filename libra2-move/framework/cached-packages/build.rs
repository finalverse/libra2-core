...
use libra2_framework::ReleaseTarget;
...
fn main() -> Result<()> {
    ...
    if std::env::var("SKIP_FRAMEWORK_BUILD").is_err() {
        ...
        println!(
            "cargo:rerun-if-changed={}",
            prev_dir.join("libra2-token").join("sources").display()
        );
        println!(
            "cargo:rerun-if-changed={}",
            prev_dir.join("libra2-token").join("Move.toml").display()
        );
        println!(
            "cargo:rerun-if-changed={}",
            prev_dir
        ...
        println!(
            "cargo:rerun-if-changed={}",
            prev_dir.join("libra2-framework").join("sources").display()
        );
        println!(
            "cargo:rerun-if-changed={}",
            prev_dir.join("libra2-framework").join("Move.toml").display()
        );
        println!(
            "cargo:rerun-if-changed={}",
