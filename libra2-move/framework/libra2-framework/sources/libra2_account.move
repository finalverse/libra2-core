module libra2_framework::libra2_account {
    use libra2_framework::account::{Self, new_event_handle};
    ...
    use libra2_framework::fungible_asset::{Self, Metadata, BurnRef, FungibleAsset};
    ...
    friend libra2_framework::resource_account;
    ...
    public entry fun transfer(source: &signer, to: address, amount: u64) {
        ...
        if (features::operations_default_to_fa_apt_store_enabled()) {
            ...
            coin::transfer<Libra2Coin>(source, to, amount)
        }
    }
    ...
    public fun deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) acquires DirectTransferConfig {
        if (!account::exists_at(to)) {
            create_account(to);
            spec {
                assert coin::spec_is_account_registered<Libra2Coin>(to);
                assume libra2_std::type_info::type_of<CoinType>() == libra2_std::type_info::type_of<Libra2Coin>() ==>
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = libra2_framework::libra2_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, bob, 500);
        assert!(coin::balance<Libra2Coin>(bob) == 500, 0);
        transfer(alice, carol, 500);
        assert!(coin::balance<Libra2Coin>(carol) == 500, 1);
        transfer(alice, carol, 1500);
        assert!(coin::balance<Libra2Coin>(carol) == 2000, 2);
    ...
    public fun test_transfer_to_resource_account(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = libra2_framework::libra2_coin::initialize_for_test(core);
        assert!(!coin::is_account_registered<Libra2Coin>(resource_acc_addr), 0);

        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, resource_acc_addr, 500);
        assert!(coin::balance<Libra2Coin>(resource_acc_addr) == 500, 1);
    ...
    public fun test_batch_transfer(from: &signer, core: &signer, recipient_1: &signer, recipient_2: &signer) {
        ...
        assert!(coin::balance<Libra2Coin>(recipient_1_addr) == 100, 0);
        assert!(coin::balance<Libra2Coin>(recipient_2_addr) == 500, 1);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }
