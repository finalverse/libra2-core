spec libra2_framework::genesis {
    ...
    spec initialize {
        ...
        ensures exists<stake::ValidatorSet>(@libra2_framework);
        ensures exists<stake::ValidatorPerformance>(@libra2_framework);
        ensures exists<storage_gas::StorageGasConfig>(@libra2_framework);
        ensures exists<storage_gas::StorageGas>(@libra2_framework);
        ensures exists<gas_schedule::GasScheduleV2>(@libra2_framework);
        ensures exists<aggregator_factory::AggregatorFactory>(@libra2_framework);
        ensures exists<coin::SupplyConfig>(@libra2_framework);
        ensures exists<chain_id::ChainId>(@libra2_framework);
        ensures exists<reconfiguration::Configuration>(@libra2_framework);
        ensures exists<block::BlockResource>(@libra2_framework);
        ensures exists<state_storage::StateStorageUsage>(@libra2_framework);
        ensures exists<timestamp::CurrentTimeMicroseconds>(@libra2_framework);
        ensures exists<account::Account>(@libra2_framework);
        ensures exists<version::SetVersionCapability>(@libra2_framework);
    ...
    spec set_genesis_end {
        ...
        /// [high-level-req-4]
        requires len(global<stake::ValidatorSet>(@libra2_framework).active_validators) >= 1;
        // property 5: The end of genesis should be marked on chain.
        /// [high-level-req-5]
        let addr = std::signer::address_of(libra2_framework);
        aborts_if addr != @libra2_framework;
        aborts_if exists<chain_status::GenesisEndMarker>(@libra2_framework);
        ensures global<chain_status::GenesisEndMarker>(@libra2_framework) == chain_status::GenesisEndMarker {};
    }

    spec schema InitalizeRequires {
        execution_config: vector<u8>;
        requires !exists<account::Account>(@libra2_framework);
    ...
    spec schema CompareTimeRequires {
        let staking_rewards_config = global<staking_config::StakingRewardsConfig>(@libra2_framework);
