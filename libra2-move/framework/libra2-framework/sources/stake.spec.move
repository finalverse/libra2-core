spec libra2_framework::stake {
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (reconfiguration_state::spec_is_in_progress() && old(exists<ValidatorSet>(@libra2_framework))) ==>
            old(global<ValidatorSet>(@libra2_framework)) == global<ValidatorSet>(@libra2_framework);
    }
    ...
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@libra2_framework);
    ...
    spec initialize_validator(
        ...
        let allowed = global<AllowedValidators>(@libra2_framework);
        aborts_if exists<ValidatorConfig>(addr);
        aborts_if exists<AllowedValidators>(@libra2_framework) && !vector::spec_contains(allowed.accounts, addr);
    ...
    )
    {
        ...
        aborts_if !exists<StakingConfig>(@libra2_framework);
        aborts_if !exists<ValidatorSet>(@libra2_framework);
        aborts_if reconfiguration_state::spec_is_in_progress();

        let stake_pool = global<StakePool>(pool_address);
        let validator_set = global<ValidatorSet>(@libra2_framework);
        let post p_validator_set = global<ValidatorSet>(@libra2_framework);
    ...
    spec leave_validator_set(
        ...
        let post post_validator_set = global<ValidatorSet>(@libra2_framework);
    ...
    spec increase_lockup_with_cap(owner_cap: &OwnerCapability) {
        let config = global<staking_config::StakingConfig>(@libra2_framework);
        let pool_address = owner_cap.pool_address;
        let pre_stake_pool = global<StakePool>(pool_address);
        let post stake_pool = global<StakePool>(pool_address);
    ...
    spec update_performance_statistics {
        ...
        let validator_perf = global<ValidatorPerformance>(@libra2_framework);
        let post post_validator_perf = global<ValidatorPerformance>(@libra2_framework);
    ...
    spec fun spec_get_reconfig_start_time_secs(): u64 {
        if (exists<reconfiguration_state::State>(@libra2_framework)) {
    ...
    spec configure_allowed_validators(libra2_framework: &signer, accounts: vector<address>) {
        let libra2_framework_address = signer::address_of(libra2_framework);
        aborts_if !system_addresses::is_libra2_framework_address(libra2_framework_address);
        let post allowed = global<AllowedValidators>(libra2_framework_address);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@libra2_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        ...
        requires exists<StakingRewardsConfig>(@libra2_framework) || !features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<timestamp::CurrentTimeMicroseconds>(@libra2_framework);
    }
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate = global<staking_config::StakingRewardsConfig>(@libra2_framework).rewards_rate;
