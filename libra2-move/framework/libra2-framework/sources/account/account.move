module libra2_framework::account {
    use std::bcs;
    use std::error;
    use std::hash;
    ...
    public(friend) fun initialize(libra2_framework: &signer) {
        ...
        move_to(libra2_framework, OriginatingAddress {
    ...
    public fun originating_address(auth_key: address): Option<address> acquires OriginatingAddress {
        let address_map_ref = &borrow_global<OriginatingAddress>(@libra2_framework).address_map;
    ...
    entry fun set_originating_address(account: &signer) acquires Account, OriginatingAddress {
        ...
        let address_map_ref_mut =
            &mut borrow_global_mut<OriginatingAddress>(@libra2_framework).address_map;
    ...
    fun update_auth_key_and_originating_address_table(
        ...
        let address_map = &mut borrow_global_mut<OriginatingAddress>(@libra2_framework).address_map;
    ...
    fun test_create_signer_for_test() {
        assert!(signer::address_of(&create_signer_for_test(@libra2_framework)) == @0x1, 0);
    ...
    #[test(alice = @0xa11ce)]
    #[expected_failure(abort_code = 65537, location = libra2_framework::ed25519)]
    ...
    //
    #[test(bob = @0x345, framework = @libra2_framework)]
    ...
    #[test(bob = @0x345, charlie = @0x567, framework = @libra2_framework)]
