spec libra2_framework::account {
    ...
    /// to the OriginatingAddress under the libra2_framework account.
    ...
    spec rotate_authentication_key(
        ...
        let address_map = global<OriginatingAddress>(@libra2_framework).address_map;
        let new_auth_key = from_bcs::deserialize<address>(new_auth_key_vector);

        aborts_if !exists<OriginatingAddress>(@libra2_framework);
        aborts_if !from_bcs::deserializable<address>(account_resource.authentication_key);
    ...
    spec offer_rotation_capability(
        ...
        let proof_challenge = RotationCapabilityOfferProofChallengeV2 {
            chain_id: global<chain_id::ChainId>(@libra2_framework).id,
            sequence_number: account_resource.sequence_number,
            source_address,
            recipient_address,
        };
        aborts_if !exists<chain_id::ChainId>(@libra2_framework);
    ...
    spec update_auth_key_and_originating_address_table(
        ...
        modifies global<OriginatingAddress>(@libra2_framework);
        include UpdateAuthKeyAndOriginatingAddressTableAbortsIf;
    }
    spec schema UpdateAuthKeyAndOriginatingAddressTableAbortsIf {
        ...
        ensures table::spec_contains(global<OriginatingAddress>(@libra2_framework).address_map, from_bcs::deserialize<address>(new_auth_key_vector));
    }
