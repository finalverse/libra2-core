...
module libra2_framework::reconfiguration {
    use std::error;
    use std::features;
    use std::signer;
    ...
    public(friend) fun initialize(libra2_framework: &signer) {
        ...
        assert!(account::get_guid_next_creation_num(signer::address_of(libra2_framework)) == 2, error::invalid_state(EINVALID_GUID_FOR_EVENT));
        move_to<Configuration>(
            libra2_framework,
            Configuration {
                epoch: 0,
                last_reconfiguration_time: 0,
                events: account::new_event_handle<NewEpochEvent>(libra2_framework),
            }
        );
    }
    ...
    fun disable_reconfiguration(libra2_framework: &signer) {
        system_addresses::assert_libra2_framework(libra2_framework);
        assert!(reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        move_to(libra2_framework, DisableReconfiguration {})
    }

    /// Private function to resume reconfiguration.
    /// This function should only be used for offline WriteSet generation purpose and should never be invoked on chain.
    fun enable_reconfiguration(libra2_framework: &signer) acquires DisableReconfiguration {
        system_addresses::assert_libra2_framework(libra2_framework);

        assert!(!reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        DisableReconfiguration {} = move_from<DisableReconfiguration>(signer::address_of(libra2_framework));
    }

    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@libra2_framework)
    }
    ...
    public(friend) fun reconfigure() acquires Configuration {
        ...
        let config_ref = borrow_global_mut<Configuration>(@libra2_framework);
    ...
    public fun last_reconfiguration_time(): u64 acquires Configuration {
        borrow_global<Configuration>(@libra2_framework).last_reconfiguration_time
    }

    public fun current_epoch(): u64 acquires Configuration {
        borrow_global<Configuration>(@libra2_framework).epoch
    }
