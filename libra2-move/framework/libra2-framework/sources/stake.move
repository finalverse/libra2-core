...
module libra2_framework::stake {
    ...
    use libra2_framework::coin::{Self, Coin, MintCapability};
    use libra2_framework::event::{Self, EventHandle};
    use libra2_framework::timestamp;
    use libra2_framework::system_addresses;
    use libra2_framework::staking_config::{Self, StakingConfig, StakingRewardsConfig};
    ...
    friend libra2_framework::reconfiguration;
    ...
    struct StakePool has key {
        // active stake
        active: Coin<Libra2Coin>,
        // inactive stake, can be withdrawn
        inactive: Coin<Libra2Coin>,
        // pending activation for next epoch
        pending_active: Coin<Libra2Coin>,
        // pending deactivation for next epoch
        pending_inactive: Coin<Libra2Coin>,
    ...
    struct ValidatorFees has key {
        fees_table: Table<address, Coin<Libra2Coin>>,
    }
    ...
    public fun get_validator_state(pool_address: address): u64 acquires ValidatorSet {
        let validator_set = borrow_global<ValidatorSet>(@libra2_framework);
    ...
    public fun get_current_epoch_proposal_counts(validator_index: u64): (u64, u64) acquires ValidatorPerformance {
        let validator_performances = &borrow_global<ValidatorPerformance>(@libra2_framework).validators;
    ...
    public(friend) fun initialize(libra2_framework: &signer) {
        ...
        move_to(libra2_framework, ValidatorSet {
            consensus_scheme: 0,
        ...
        move_to(libra2_framework, ValidatorPerformance {
    ...
    public(friend) fun store_libra2_coin_mint_cap(libra2_framework: &signer, mint_cap: MintCapability<Libra2Coin>) {
        system_addresses::assert_libra2_framework(libra2_framework);
        move_to(libra2_framework, Libra2CoinCapabilities { mint_cap })
    }
    ...
    public fun remove_validators(
        ...
        system_addresses::assert_libra2_framework(libra2_framework);
        let validator_set = borrow_global_mut<ValidatorSet>(@libra2_framework);
    ...
    fun initialize_owner(owner: &signer) acquires AllowedValidators {
        ...
        move_to(owner, StakePool {
            active: coin::zero<Libra2Coin>(),
            pending_active: coin::zero<Libra2Coin>(),
            pending_inactive: coin::zero<Libra2Coin>(),
            inactive: coin::zero<Libra2Coin>(),
            locked_until_secs: 0,
    ...
    public entry fun add_stake(owner: &signer, amount: u64) acquires OwnerCapability, StakePool, ValidatorSet {
        ...
        assert_owner_cap_exists(owner_address);
        let ownership_cap = borrow_global<OwnerCapability>(owner_address);
        add_stake_with_cap(ownership_cap, coin::withdraw<Libra2Coin>(owner, amount));
    }

    /// Add `coins` into `pool_address`. this requires the corresponding `owner_cap` to be passed in.
    public fun add_stake_with_cap(owner_cap: &OwnerCapability, coins: Coin<Libra2Coin>) acquires StakePool, ValidatorSet {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        ...
        if (is_current_epoch_validator(pool_address)) {
            coin::merge<Libra2Coin>(&mut stake_pool.pending_active, coins);
        } else {
            coin::merge<Libra2Coin>(&mut stake_pool.active, coins);
        };
    ...
    public fun unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) acquires StakePool {
        ...
        coin::merge<Libra2Coin>(&mut stake_pool.pending_inactive, unlocked_stake);
    ...
    public entry fun withdraw(
        ...
        coin::deposit<Libra2Coin>(owner_address, coins);
    }
    ...
    public fun withdraw_with_cap(
        ...
        withdraw_amount: u64
    ): Coin<Libra2Coin> acquires StakePool, ValidatorSet {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        ...
        if (withdraw_amount == 0) return coin::zero<Libra2Coin>();
    ...
    public(friend) fun update_performance_statistics(
        ...
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@libra2_framework);
    ...
    ) acquires StakePool, Libra2CoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        let validator_set = borrow_global_mut<ValidatorSet>(@libra2_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@libra2_framework);
    ...
    public fun next_validator_consensus_infos(): vector<ValidatorConsensusInfo> acquires ValidatorSet, ValidatorPerformance, StakePool, ValidatorConfig {
        // Init.
        let cur_validator_set = borrow_global<ValidatorSet>(@libra2_framework);
        let staking_config = staking_config::get();
        let validator_perf = borrow_global<ValidatorPerformance>(@libra2_framework);
    ...
    ) acquires StakePool, Libra2CoinCapabilities, ValidatorConfig {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let validator_config = borrow_global<ValidatorConfig>(pool_address);
    ...
    fun distribute_rewards(
        stake: &mut Coin<Libra2Coin>,
        num_successful_proposals: u64,
        num_total_proposals: u64,
        rewards_rate: u64,
        rewards_rate_denominator: u64,
    ): u64 acquires Libra2CoinCapabilities {
    ...
    public fun configure_allowed_validators(
        ...
        let libra2_framework_address = signer::address_of(libra2_framework);
        system_addresses::assert_libra2_framework(libra2_framework);
        if (!exists<AllowedValidators>(libra2_framework_address)) {
            move_to(libra2_framework, AllowedValidators { accounts });
        } else {
            let allowed = borrow_global_mut<AllowedValidators>(libra2_framework_address);
            allowed.accounts = accounts;
        }
    }

    fun is_allowed(account: address): bool acquires AllowedValidators {
        if (!exists<AllowedValidators>(@libra2_framework)) {
            true
        } else {
            let allowed = borrow_global<AllowedValidators>(@libra2_framework);
    ...
    public fun initialize_for_test(libra2_framework: &signer) {
        reconfiguration_state::initialize(libra2_framework);
        initialize_for_test_custom(libra2_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 1000000);
    ...
    ) acquires Libra2CoinCapabilities, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    public fun initialize_for_test_custom(
        ...
        reconfiguration_state::initialize(libra2_framework);
        if (!exists<ValidatorSet>(@libra2_framework)) {
            initialize(libra2_framework);
        };
        ...
        if (!exists<Libra2CoinCapabilities>(@libra2_framework)) {
            let (burn_cap, mint_cap) = libra2_coin::initialize_for_test(libra2_framework);
            store_libra2_coin_mint_cap(libra2_framework, mint_cap);
            coin::destroy_burn_cap<Libra2Coin>(burn_cap);
        };
    ...
    #[test_only]
    public fun mint(account: &signer, amount: u64) acquires Libra2CoinCapabilities {
    ...
    public fun mint_and_add_stake(
        account: &signer, amount: u64) acquires Libra2CoinCapabilities, OwnerCapability, StakePool, ValidatorSet {
        mint(account, amount);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool,
    ) acquires AllowedValidators, Libra2CoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    #[test(libra2_framework = @libra2_framework, validator = @0x123)]
    ...
    #[test(libra2_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_for_test_custom(libra2_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100000);
    ...
    #[test(libra2_framework = @libra2_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    ...
    public entry fun test_end_to_end(
        libra2_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, Libra2CoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        initialize_for_test(libra2_framework);
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100, true, true);
        ...
        assert!(coin::balance<Libra2Coin>(validator_address) == 800, 2);
        ...
        assert!(coin::balance<Libra2Coin>(validator_address) == 850, 6);
        assert_validator_state(validator_address, 102, 51, 0, 0, 0);
        withdraw(validator, 51);
        assert!(coin::balance<Libra2Coin>(validator_address) == 901, 7);
    ...
    public entry fun test_cannot_reduce_lockup(
        ...
        staking_config::update_recurring_lockup_duration_secs(libra2_framework, 1);
    ...
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        initialize_for_test_custom(libra2_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 50);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        initialize_for_test_custom(libra2_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 10000);
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        initialize_for_test_custom(libra2_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100);
        // Need 1 validator to be in the active validator set so joining limit works.
        let (_sk_1, pk_1, pop_1) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, true);
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        assert!(borrow_global<ValidatorSet>(@libra2_framework).total_joining_power == 100, 0);
        ...
        assert!(borrow_global<ValidatorSet>(@libra2_framework).total_joining_power == 0, 1);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        assert!(coin::balance<Libra2Coin>(validator_address) == 1001, 2);
    ...
    #[test(libra2_framework = @libra2_framework, validator = @0x123, validator_2 = @0x234)]
    ...
    #[test(libra2_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234, validator_3 = @0x345)]
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_for_test_custom(libra2_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@libra2_framework).pending_inactive,
                0
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@libra2_framework).pending_active,
                0
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@libra2_framework).active_validators,
                0
    ...
    public entry fun test_validator_cannot_join_post_genesis(
        ...
        initialize_for_test_custom(libra2_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, false, 1, 100, 100);
    ...
    #[test(
        libra2_framework = @libra2_framework,
        validator_1 = @libra2_framework,
        validator_2 = @0x2,
        validator_3 = @0x3,
        validator_4 = @0x4,
        validator_5 = @0x5
    ...
    fun test_validator_consensus_infos_from_validator_set(
        ...
        end_epoch();
        let vci_vec_0 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@libra2_framework));
        ...
        assert!(vector[@0x5, @libra2_framework, @0x3] == vci_addrs, 1);
        ...
        let vci_vec_1 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@libra2_framework));
        assert!(vci_vec_0 == vci_vec_1, 11);
        join_validator_set(validator_2, v2_addr);
        let vci_vec_2 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@libra2_framework));
        assert!(vci_vec_0 == vci_vec_2, 12);
        leave_validator_set(validator_1, v1_addr);
        let vci_vec_3 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@libra2_framework));
        assert!(vci_vec_0 == vci_vec_3, 13);
        join_validator_set(validator_4, v4_addr);
        let vci_vec_4 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@libra2_framework));
    ...
    public entry fun test_validator_rewards_rate_decrease_over_time(
        ...
        features::change_feature_flags_for_testing(libra2_framework, vector[features::get_periodical_reward_rate_decrease_feature()], vector[]);
    ...
    public entry fun test_invalid_config(
        libra2_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, Libra2CoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorSet {
        initialize_for_test_custom(libra2_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100, true, true);
        assert!(vector::length(&borrow_global<ValidatorSet>(@libra2_framework).active_validators) == 2, 0);
        ...
        remove_validators(libra2_framework, &vector[validator_to_remove]);
        assert!(vector::length(&borrow_global<ValidatorSet>(@libra2_framework).active_validators) == 1, 0);
    ...
    #[test(libra2_framework = @0x1, validator = @0x123)]
    public entry fun test_allowed_validators(
        libra2_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, ValidatorSet {
        let addr = signer::address_of(validator);
        let (burn, mint) = libra2_coin::initialize_for_test(libra2_framework);
        configure_allowed_validators(libra2_framework, vector[addr]);
    ...
    public entry fun test_not_allowed_validators(
        ...
        configure_allowed_validators(libra2_framework, vector[]);
        let (burn, mint) = libra2_coin::initialize_for_test(libra2_framework);
