spec libra2_framework::block {
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<BlockResource>(@libra2_framework);
        // After genesis, `CommitHistory` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<CommitHistory>(@libra2_framework);
    }
    ...
    spec emit_genesis_block_event {
        ...
        requires event::counter(global<BlockResource>(@libra2_framework).new_block_events) == 0;
    ...
    spec initialize(libra2_framework: &signer, epoch_interval_microsecs: u64) {
        use std::signer;
        /// [high-level-req-1]
        include Initialize;
    ...
    spec get_epoch_interval_secs(): u64 {
        aborts_if !exists<BlockResource>(@libra2_framework);
    }
    ...
    spec schema EmitWritesetBlockEvent {
        ...
        aborts_if !exists<BlockResource>(@libra2_framework);
        aborts_if !exists<reconfiguration::Configuration>(@libra2_framework);
