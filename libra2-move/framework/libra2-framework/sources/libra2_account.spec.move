spec libra2_framework::libra2_account {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: After creating an Aptos account, the account should become registered to receive Libra2Coin.
    /// Criticality: Critical
    ...
    /// exist; then, register Libra2Coin for that account if a particular is unregistered before transferring the amount.
    /// Criticality: Critical
    /// Implementation: The transfer function checks if the recipient account exists. If the account does not exist,
    /// the function creates one and registers the account to Libra2Coin if not registered.
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @libra2_framework || auth_key == @libra2_token;
    }
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        include GuidAbortsIf<Libra2Coin>;
        include WithdrawAbortsIf<Libra2Coin>{from: source};
        include TransferEnsures<Libra2Coin>;

        aborts_if exists<coin::CoinStore<Libra2Coin>>(to) && global<coin::CoinStore<Libra2Coin>>(to).frozen;
        /// [high-level-req-5]
        ensures exists<libra2_framework::account::Account>(to);
        ensures exists<coin::CoinStore<Libra2Coin>>(to);
    }
    ...
    spec assert_account_is_registered_for_apt(addr: address) {
        pragma aborts_if_is_partial;
        aborts_if !account::exists_at(addr);
        aborts_if !coin::spec_is_account_registered<Libra2Coin>(addr);
    }
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        pragma verify = false;
        let account_addr_source = signer::address_of(source);
        let coin_store_source = global<coin::CoinStore<Libra2Coin>>(account_addr_source);
        ...
        aborts_if exists i in 0..len(recipients):
                !account::exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @libra2_framework || recipients[i] == @libra2_token);
        ...
        aborts_if exists i in 0..len(recipients):
            !exists<coin::CoinStore<Libra2Coin>>(account_addr_source);
        aborts_if exists i in 0..len(recipients):
            coin_store_source.frozen;
        aborts_if exists i in 0..len(recipients):
            global<coin::CoinStore<Libra2Coin>>(account_addr_source).coin.value < amounts[i];

        // deposit properties
        aborts_if exists i in 0..len(recipients):
            exists<coin::CoinStore<Libra2Coin>>(recipients[i]) && global<coin::CoinStore<Libra2Coin>>(recipients[i]).frozen;

        // guid properties
        aborts_if exists i in 0..len(recipients):
            account::exists_at(recipients[i]) && !exists<coin::CoinStore<Libra2Coin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        aborts_if exists i in 0..len(recipients):
            account::exists_at(recipients[i]) && !exists<coin::CoinStore<Libra2Coin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 > MAX_U64;
    }
    ...
    spec deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) {
        ...
        /// [high-level-spec-6]
        ensures exists<libra2_framework::account::Account>(to);
        ensures exists<libra2_framework::coin::CoinStore<CoinType>>(to);
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::exists_at(to) && (to == @vm_reserved || to == @libra2_framework || to == @libra2_token);
    }
    ...
    spec schema RegistCoinAbortsIf<CoinType> {
        ...
        aborts_if type_info::type_of<CoinType>() != type_info::type_of<Libra2Coin>();
    }
