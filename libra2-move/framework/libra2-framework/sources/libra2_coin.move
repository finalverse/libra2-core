...
module libra2_framework::libra2_coin {
    use std::error;
    use std::signer;
    use std::string;
    use std::vector;
    ...
    use libra2_framework::coin::{Self, BurnCapability, MintCapability};
    ...
    public(friend) fun initialize(libra2_framework: &signer): (BurnCapability<Libra2Coin>, MintCapability<Libra2Coin>) {
    ...
    public(friend) fun destroy_mint_cap(libra2_framework: &signer) acquires MintCapStore {
        system_addresses::assert_libra2_framework(libra2_framework);
        let MintCapStore { mint_cap } = move_from<MintCapStore>(@libra2_framework);
    ...
    public(friend) fun configure_accounts_for_test(
        ...
        // Mint the core resource account Libra2Coin for gas so it can execute system transactions.
        let coins = coin::mint<Libra2Coin>(
            18446744073709551615,
            &mint_cap,
        );
        coin::deposit<Libra2Coin>(signer::address_of(core_resources), coins);
    ...
    public entry fun mint(
        ...
        let coins_minted = coin::mint<Libra2Coin>(amount, mint_cap);
        coin::deposit<Libra2Coin>(dst_addr, coins_minted);
    }
    ...
    #[test_only]
    use libra2_framework::fungible_asset::FungibleAsset;
    ...
    public fun mint_apt_fa_for_test(amount: u64): FungibleAsset acquires MintCapStore {
        ...
        coin::coin_to_fungible_asset(
            coin::mint(
                amount,
                &borrow_global<MintCapStore>(@libra2_framework).mint_cap
            )
        )
    }
    ...
    public fun ensure_initialized_with_apt_fa_metadata_for_test() {
        let libra2_framework = account::create_signer_for_test(@libra2_framework);
        if (!exists<MintCapStore>(@libra2_framework)) {
            ...
            let (burn_cap, mint_cap) = initialize(&libra2_framework);
    ...
    #[test_only]
    public fun initialize_for_test(libra2_framework: &signer): (BurnCapability<Libra2Coin>, MintCapability<Libra2Coin>) {
        aggregator_factory::initialize_aggregator_factory_for_test(libra2_framework);
        let (burn_cap, mint_cap) = initialize(libra2_framework);
        coin::create_coin_conversion_map(libra2_framework);
        coin::create_pairing<Libra2Coin>(libra2_framework);
    ...
    public fun initialize_for_test_without_aggregator_factory(
        libra2_framework: &signer
    ): (BurnCapability<Libra2Coin>, MintCapability<Libra2Coin>) {
        let (burn_cap, mint_cap) = initialize(libra2_framework);
        coin::create_coin_conversion_map(libra2_framework);
        coin::create_pairing<Libra2Coin>(libra2_framework);
        (burn_cap, mint_cap)
