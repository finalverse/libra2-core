module libra2_framework::genesis {
    use std::error;
    ...
    use libra2_framework::staking_contract;
    use libra2_framework::staking_config;
    use libra2_framework::state_storage;
    ...
    fun initialize(
        ...
        execution_config::set(&libra2_framework_account, execution_config);
        version::initialize(&libra2_framework_account, initial_version);
        stake::initialize(&libra2_framework_account);
        timestamp::set_time_has_started(&libra2_framework_account);
        ...
        storage_gas::initialize(&libra2_framework_account);
        gas_schedule::initialize(&libra2_framework_account, gas_schedule);

        // Ensure we can create aggregators for supply, but not enable it for common use just yet.
        aggregator_factory::initialize_aggregator_factory(&libra2_framework_account);

        chain_id::initialize(&libra2_framework_account, chain_id);
        reconfiguration::initialize(&libra2_framework_account);
        block::initialize(&libra2_framework_account, epoch_interval_microsecs);
        state_storage::initialize(&libra2_framework_account);
    }
    ...
    fun create_account(libra2_framework: &signer, account_address: address, balance: u64): signer {
    ...
    fun create_initialize_validators(libra2_framework: &signer, validators: vector<ValidatorConfiguration>) {
        ...
        create_initialize_validators_with_commission(libra2_framework, false, validators_with_commission);
    }
    ...
    fun create_initialize_validator(
        ...
        let owner = &create_account(libra2_framework, validator.owner_address, validator.stake_amount);
        create_account(libra2_framework, validator.operator_address, 0);
        create_account(libra2_framework, validator.voter_address, 0);
    ...
    fun set_genesis_end(libra2_framework: &signer) {
        chain_status::set_genesis_end(libra2_framework);
    }
    ...
    fun initialize_for_verification(
        ...
        create_initialize_validators_with_commission(libra2_framework, true, validators);
        set_genesis_end(libra2_framework);
    }
    ...
    fun test_setup() {
        setup();
        assert!(account::exists_at(@libra2_framework), 1);
    ...
    fun test_create_root_account(libra2_framework: &signer) {
        use libra2_framework::aggregator_factory;
        ...
        features::change_feature_flags_for_testing(libra2_framework, vector[feature], vector[]);
