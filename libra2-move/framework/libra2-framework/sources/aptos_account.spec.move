spec libra2_framework::aptos_account {
    /// <high-level-req>
    /// No.: 1
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @libra2_framework || auth_key == @aptos_token;
    }
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        aborts_if exists i in 0..len(recipients):
                !account::exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @libra2_framework || recipients[i] == @aptos_token);
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::exists_at(to) && (to == @vm_reserved || to == @libra2_framework || to == @aptos_token);
    }
