...
module libra2_framework::vesting {
    use std::bcs;
    use std::error;
    ...
    const VESTING_POOL_SALT: vector<u8> = b"libra2_framework::vesting";
    ...
    public fun setup(libra2_framework: &signer, accounts: &vector<address>) {
        ...
        std::features::change_feature_flags_for_testing(libra2_framework, vector[], vector[std::features::get_periodical_reward_rate_decrease_feature()]);
    }
    ...
    #[test(libra2_framework = @0x1, admin = @0x123, shareholder_1 = @0x234, shareholder_2 = @0x345, withdrawal = @111)]
    public entry fun test_end_to_end(
        ...
        setup(
            libra2_framework, &vector[admin_address, withdrawal_address, shareholder_1_address, shareholder_2_address]);
    ...
    public entry fun test_create_vesting_contract_with_zero_grant_should_fail(
        ...
        let admin_address = signer::address_of(admin);
        setup(libra2_framework, &vector[admin_address]);
    ...
    public entry fun test_create_empty_vesting_schedule_should_fail(libra2_framework: &signer) {
        setup(libra2_framework, &vector[]);
        create_vesting_schedule(vector[], 1, 1);
    }

    #[test(libra2_framework = @0x1)]
    #[expected_failure(abort_code = 0x10003, location = Self)]
    public entry fun test_create_vesting_schedule_with_zero_period_duration_should_fail(libra2_framework: &signer) {
        setup(libra2_framework, &vector[]);
    ...
    public entry fun test_create_vesting_schedule_with_invalid_vesting_start_should_fail(libra2_framework: &signer) {
        setup(libra2_framework, &vector[]);
    ...
    public entry fun test_vest_twice_should_not_double_count(
        ...
        setup(libra2_framework, &vector[admin_address, shareholder_address]);
    ...
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        let shareholder_address = signer::address_of(shareholder);
        setup(libra2_framework, &vector[admin_address, shareholder_address, operator_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
    ...
    public entry fun test_update_operator_with_same_commission(
        ...
        let admin_address = signer::address_of(admin);
        let operator_address = signer::address_of(operator);
        setup(libra2_framework, &vector[admin_address, @11, operator_address]);
        let contract_address = setup_vesting_contract(
    ...
    public entry fun test_set_beneficiary_for_operator(
        ...
        setup(libra2_framework, &vector[admin_address, shareholder_address, operator_address1, beneficiary_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        assert!(operator_commission_percentage(contract_address) == 0, 0);
        let stake_pool_address = stake_pool_address(contract_address);
    ...
    public entry fun test_set_beneficiary_should_send_distribution(
        ...
        let admin_address = signer::address_of(admin);
        setup(libra2_framework, &vector[admin_address, @11]);
        let contract_address = setup_vesting_contract(
    ...
    public entry fun test_reset_beneficiary(
        ...
        let admin_address = signer::address_of(admin);
        setup(libra2_framework, &vector[admin_address, @11, @12]);
        let contract_address = setup_vesting_contract(
            admin, &vector[@11], &vector[GRANT_AMOUNT], admin_address, 0);
    ...
    #[test(libra2_framework = @0x1, admin = @0x123, resetter = @0x234, random = @0x345)]
