/// Provides a common place for exporting `create_signer` across the Libra2 Framework.
///
/// To use create_signer, add the module below, such that:
/// `friend libra2_framework::friend_wants_create_signer`
/// where `friend_wants_create_signer` is the module that needs `create_signer`.
///
/// Note, that this is only available within the Libra2 Framework.
///
/// This exists to make auditing straight forward and to limit the need to depend
/// on account to have access to this.
module libra2_framework::create_signer {
    friend libra2_framework::account;
    friend libra2_framework::aptos_account;
    friend libra2_framework::coin;
    friend libra2_framework::fungible_asset;
    friend libra2_framework::genesis;
    friend libra2_framework::account_abstraction;
    friend libra2_framework::multisig_account;
    friend libra2_framework::object;
    friend libra2_framework::permissioned_signer;
    friend libra2_framework::transaction_validation;

    public(friend) native fun create_signer(addr: address): signer;
}
