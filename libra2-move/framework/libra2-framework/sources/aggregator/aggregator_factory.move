...
module libra2_framework::aggregator_factory {
    use std::error;
    ...
    public(friend) fun create_aggregator_internal(): Aggregator acquires AggregatorFactory {
        assert!(
            exists<AggregatorFactory>(@libra2_framework),
            error::not_found(EAGGREGATOR_FACTORY_NOT_FOUND)
        );

        let aggregator_factory = borrow_global_mut<AggregatorFactory>(@libra2_framework);
    ...
    #[test_only]
    public fun initialize_aggregator_factory_for_test(libra2_framework: &signer) {
        initialize_aggregator_factory(libra2_framework);
    }
