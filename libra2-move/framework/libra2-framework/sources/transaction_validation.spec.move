spec libra2_framework::transaction_validation {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Ensure caller is `libra2_framework`.
    ...
    spec schema PrologueCommonAbortsIf {
        use std::bcs;
        use libra2_framework::timestamp::{CurrentTimeMicroseconds};
        use libra2_framework::chain_id::{ChainId};
        use libra2_framework::account::{Account};
        use libra2_framework::coin::{CoinStore};
        sender: &signer;
        gas_payer: &signer;
        ...
        aborts_if !exists<CurrentTimeMicroseconds>(@libra2_framework);
