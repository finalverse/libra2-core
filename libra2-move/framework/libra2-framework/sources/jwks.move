...
module libra2_framework::jwks {
    ...
    public fun try_get_patched_jwk(issuer: vector<u8>, jwk_id: vector<u8>): Option<JWK> acquires PatchedJWKs {
        let jwks = &borrow_global<PatchedJWKs>(@libra2_framework).jwks;
    ...
    public fun upsert_oidc_provider(fx: &signer, name: vector<u8>, config_url: vector<u8>): Option<vector<u8>> acquires SupportedOIDCProviders {
        ...
        let provider_set = borrow_global_mut<SupportedOIDCProviders>(@libra2_framework);
    ...
    public fun upsert_oidc_provider_for_next_epoch(fx: &signer, name: vector<u8>, config_url: vector<u8>): Option<vector<u8>> acquires SupportedOIDCProviders {
        ...
        let provider_set = if (config_buffer::does_exist<SupportedOIDCProviders>()) {
            ...
            *borrow_global<SupportedOIDCProviders>(@libra2_framework)
        };
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires SupportedOIDCProviders {
        ...
        if (config_buffer::does_exist<SupportedOIDCProviders>()) {
            ...
            if (exists<SupportedOIDCProviders>(@libra2_framework)) {
                *borrow_global_mut<SupportedOIDCProviders>(@libra2_framework) = new_config;
            } else {
    ...
    public fun set_patches(fx: &signer, patches: vector<Patch>) acquires Patches, PatchedJWKs, ObservedJWKs {
        system_addresses::assert_libra2_framework(fx);
        borrow_global_mut<Patches>(@libra2_framework).patches = patches;
    ...
    public fun upsert_into_observed_jwks(fx: &signer, provider_jwks_vec: vector<ProviderJWKs>) acquires ObservedJWKs, PatchedJWKs, Patches {
        system_addresses::assert_libra2_framework(fx);
        let observed_jwks = borrow_global_mut<ObservedJWKs>(@libra2_framework);
    ...
    fun regenerate_patched_jwks() acquires PatchedJWKs, Patches, ObservedJWKs {
        let jwks = borrow_global<ObservedJWKs>(@libra2_framework).jwks;
        let patches = borrow_global<Patches>(@libra2_framework);
        vector::for_each_ref(&patches.patches, |obj|{
            let patch: &Patch = obj;
            apply_patch(&mut jwks, *patch);
        });
        *borrow_global_mut<PatchedJWKs>(@libra2_framework) = PatchedJWKs { jwks };
    }
    ...
    #[test_only]
    fun initialize_for_test(libra2_framework: &signer) {
        create_account_for_test(@libra2_framework);
        reconfiguration::initialize_for_test(libra2_framework);
        initialize(libra2_framework);
    }

    #[test(fx = @libra2_framework)]
    fun test_observed_jwks_operations(fx: &signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@libra2_framework).jwks, 1);
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@libra2_framework).jwks, 2);
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@libra2_framework).jwks, 3);

        remove_issuer_from_observed_jwks(fx, b"alice");
        let expected = AllProvidersJWKs { entries: vector[bob_jwks_v1] };
        assert!(expected == borrow_global<ObservedJWKs>(@libra2_framework).jwks, 4);
    }
    ...
    fun test_patched_jwks(libra2_framework: signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        upsert_into_observed_jwks(&libra2_framework, vector [
            ProviderJWKs {
                issuer: b"alice",
                version: 111,
                jwks: vector[jwk_0, jwk_1],
        ...
        set_patches(&libra2_framework, vector[
