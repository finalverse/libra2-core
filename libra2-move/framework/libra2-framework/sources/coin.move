...
module libra2_framework::coin {
    ...
    use libra2_framework::optional_aggregator::{Self, OptionalAggregator};
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@libra2_framework) && features::coin_to_fungible_asset_migration_feature_enabled(
        )) {
            let map = &borrow_global<CoinConversionMap>(@libra2_framework).coin_to_fungible_asset_map;
    ...
    public entry fun create_coin_conversion_map(libra2_framework: &signer) {
        system_addresses::assert_libra2_framework(libra2_framework);
        if (!exists<CoinConversionMap>(@libra2_framework)) {
            move_to(libra2_framework, CoinConversionMap {
    ...
    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(allow_apt_creation: bool): Object<Metadata> {
        ...
        assert!(exists<CoinConversionMap>(@libra2_framework), error::not_found(ECOIN_CONVERSION_MAP_NOT_FOUND));
    ...
    public fun allow_supply_upgrades(_libra2_framework: &signer, _allowed: bool) {
    ...
    fun mint_internal<CoinType>(amount: u64): Coin<CoinType> acquires CoinInfo {
        ...
        if (option::is_some(maybe_supply)) {
            ...
            spec {
                ...
                use libra2_framework::aggregator;
    ...
    #[test(source = @0x2, framework = @libra2_framework)]
    ...
    #[test(account = @0x1)]
    #[expected_failure(abort_code = 0x50003, location = libra2_framework::fungible_asset)]
    ...
    #[test(framework = @libra2_framework, other = @0x123)]
    ...
    #[test(framework = @libra2_framework)]
    ...
    #[test(framework = @libra2_framework)]
    #[expected_failure(abort_code = 0x20001, location = libra2_framework::aggregator)]
    ...
    #[test(account = @libra2_framework, aaron = @0xcafe)]
    ...
    #[test(account = @libra2_framework, aaron = @0xaa10, bob = @0xb0b)]
    ...
    #[test(account = @libra2_framework)]
    #[expected_failure(abort_code = 0x50024, location = libra2_framework::fungible_asset)]
