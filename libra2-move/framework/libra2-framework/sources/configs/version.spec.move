spec libra2_framework::version {
    ...
    spec set_version(account: &signer, major: u64) {
        ...
        let old_major = global<Version>(@libra2_framework).major;
        /// [high-level-req-2]
    ...
    spec initialize(libra2_framework: &signer, initial_version: u64) {
        ...
        /// [high-level-req-1]
        aborts_if signer::address_of(libra2_framework) != @libra2_framework;
        aborts_if exists<Version>(@libra2_framework);
        aborts_if exists<SetVersionCapability>(@libra2_framework);
        ensures exists<Version>(@libra2_framework);
        ensures exists<SetVersionCapability>(@libra2_framework);
        ensures global<Version>(@libra2_framework) == Version { major: initial_version };
        ensures global<SetVersionCapability>(@libra2_framework) == SetVersionCapability {};
    }

    spec set_for_next_epoch(account: &signer, major: u64) {
        aborts_if !exists<SetVersionCapability>(signer::address_of(account));
        aborts_if !exists<Version>(@libra2_framework);
        aborts_if global<Version>(@libra2_framework).major >= major;
        aborts_if !exists<config_buffer::PendingConfigs>(@libra2_framework);
    }
