...
module libra2_framework::block {
    ...
    public(friend) fun initialize(libra2_framework: &signer, epoch_interval_microsecs: u64) {
        ...
        move_to<CommitHistory>(libra2_framework, CommitHistory {
            max_capacity: 2000,
            next_idx: 0,
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                new_block_events: account::new_event_handle<NewBlockEvent>(libra2_framework),
                update_epoch_interval_events: account::new_event_handle<UpdateEpochIntervalEvent>(libra2_framework),
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        ...
        let block_resource = borrow_global_mut<BlockResource>(@libra2_framework);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@libra2_framework).epoch_interval / 1000000
    }
    ...
    fun block_prologue_common(
        ...
        let block_metadata_ref = borrow_global_mut<BlockResource>(@libra2_framework);
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@libra2_framework).height
    }
    ...
    fun emit_new_block_event(
        ...
        if (exists<CommitHistory>(@libra2_framework)) {
            let commit_history_ref = borrow_global_mut<CommitHistory>(@libra2_framework);
    ...
    public entry fun test_update_epoch_interval(libra2_framework: signer) acquires BlockResource {
        account::create_account_for_test(@libra2_framework);
        initialize(&libra2_framework, 1);
        assert!(borrow_global<BlockResource>(@libra2_framework).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&libra2_framework, 2);
        assert!(borrow_global<BlockResource>(@libra2_framework).epoch_interval == 2, 1);
    }

    #[test(libra2_framework = @libra2_framework, account = @0x123)]
