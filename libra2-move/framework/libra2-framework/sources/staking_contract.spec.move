spec libra2_framework::staking_contract {
    /// <high-level-req>
    /// No.: 1
    ...
    spec schema PreconditionsInCreateContract {
        requires exists<stake::ValidatorPerformance>(@libra2_framework);
        requires exists<stake::ValidatorSet>(@libra2_framework);
    ...
    spec schema CreateStakePoolAbortsIf {
        ...
        let allowed = global<stake::AllowedValidators>(@libra2_framework);
        aborts_if exists<stake::AllowedValidators>(@libra2_framework) && !contains(allowed.accounts, resource_addr);
