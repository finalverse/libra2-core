spec libra2_framework::libra2_governance {
    /// <high-level-req>
    /// No.: 1
    ...
    spec store_signer_cap(
        ...
        aborts_if !system_addresses::is_libra2_framework_address(signer::address_of(libra2_framework));
        aborts_if !system_addresses::is_framework_reserved_address(signer_address);

        let signer_caps = global<GovernanceResponsbility>(@libra2_framework).signer_caps;
        aborts_if exists<GovernanceResponsbility>(@libra2_framework) &&
            simple_map::spec_contains_key(signer_caps, signer_address);
        ensures exists<GovernanceResponsbility>(@libra2_framework);
        let post post_signer_caps = global<GovernanceResponsbility>(@libra2_framework).signer_caps;
    ...
    spec initialize_partial_voting(
        ...
        aborts_if exists<VotingRecordsV2>(@libra2_framework);
        ensures exists<VotingRecordsV2>(@libra2_framework);
    }
    ...
    spec update_governance_config(
        ...
        let addr = signer::address_of(libra2_framework);
        let governance_config = global<GovernanceConfig>(@libra2_framework);

        let post new_governance_config = global<GovernanceConfig>(@libra2_framework);
        aborts_if addr != @libra2_framework;
        aborts_if !exists<GovernanceConfig>(@libra2_framework);
    ...
    spec has_entirely_voted(stake_pool: address, proposal_id: u64): bool {
        aborts_if !exists<VotingRecords>(@libra2_framework);
    }
    ...
    spec schema CreateProposalAbortsIf {
        ...
        let staking_config = global<staking_config::StakingConfig>(@libra2_framework);
        ...
        let addr = libra2_std::type_info::type_of<Libra2Coin>().account_address;
        aborts_if !exists<coin::CoinInfo<Libra2Coin>>(addr);
        let maybe_supply = global<coin::CoinInfo<Libra2Coin>>(addr).supply;
        let supply = option::spec_borrow(maybe_supply);
        let total_supply = libra2_framework::optional_aggregator::optional_aggregator_value(supply);
        ...
        aborts_if !exists<voting::VotingForum<GovernanceProposal>>(@libra2_framework);
        let voting_forum = global<voting::VotingForum<GovernanceProposal>>(@libra2_framework);
        let proposal_id = voting_forum.next_proposal_id;
        aborts_if proposal_id + 1 > MAX_U64;
        let post post_voting_forum = global<voting::VotingForum<GovernanceProposal>>(@libra2_framework);
        ...
        aborts_if !exists<GovernanceEvents>(@libra2_framework);
    }
    ...
    spec schema VoteAbortIf {
        ...
        aborts_if spec_proposal_expiration <= locked_until && !exists<timestamp::CurrentTimeMicroseconds>(@libra2_framework);
        let spec_proposal_expiration = voting::spec_get_proposal_expiration_secs<GovernanceProposal>(@libra2_framework, proposal_id);
        ...
        aborts_if !remain_zero_1_cond && !exists<VotingRecords>(@libra2_framework);
        ...
        aborts_if !exists<VotingRecords>(@libra2_framework);
        let voting_records = global<VotingRecords>(@libra2_framework);


        // verify get_voting_power(stake_pool)
        let allow_validator_set_change = global<staking_config::StakingConfig>(@libra2_framework).allow_validator_set_change;
        let stake_pool_res = global<stake::StakePool>(stake_pool);
        ...
        let post post_approved_hashes = global<ApprovedExecutionHashes>(@libra2_framework);

        // Due to the complexity of the success state, the validation of 'borrow_global_mut<ApprovedExecutionHashes>(@libra2_framework);' is discussed in four cases.
        /// [high-level-req-3]
        aborts_if
            if (should_pass) {
                proposal_state_successed_0 && !exists<ApprovedExecutionHashes>(@libra2_framework)
            } else {
                proposal_state_successed_1 && !exists<ApprovedExecutionHashes>(@libra2_framework)
            };
        aborts_if
            if (should_pass) {
                proposal_state_successed_2 && !exists<ApprovedExecutionHashes>(@libra2_framework)
            } else {
                proposal_state_successed_3 && !exists<ApprovedExecutionHashes>(@libra2_framework)
            };
        ...
        aborts_if features::spec_partial_governance_voting_enabled() && !exists<VotingRecordsV2>(@libra2_framework);
    }
    ...
    spec schema AddApprovedScriptHash {
        proposal_id: u64;
        aborts_if !exists<ApprovedExecutionHashes>(@libra2_framework);
    ...
    spec resolve(proposal_id: u64, signer_address: address): signer {
        ...
        aborts_if !exists<ApprovedExecutionHashes>(@libra2_framework);
        let post post_approved_hashes = global<ApprovedExecutionHashes>(@libra2_framework).hashes;
        ensures !simple_map::spec_contains_key(post_approved_hashes, proposal_id);

        // verify get_signer
        include GetSignerAbortsIf;
        let governance_responsibility = global<GovernanceResponsbility>(@libra2_framework);
    ...
    spec remove_approved_hash(proposal_id: u64) {
        ...
        let proposal = table::spec_get(voting_forum.proposals, proposal_id);
        aborts_if !proposal.is_resolved;
        let post approved_hashes = global<ApprovedExecutionHashes>(@libra2_framework).hashes;
        ensures !simple_map::spec_contains_key(approved_hashes, proposal_id);
    }

    spec reconfigure(libra2_framework: &signer) {
        use libra2_framework::chain_status;
        use libra2_framework::coin::CoinInfo;
        use libra2_framework::libra2_coin::Libra2Coin;
        pragma verify = false; // TODO: set because of timeout (property proved).
    ...
    spec get_signer_testnet_only(core_resources: &signer, signer_address: address): signer {
        ...
        aborts_if !exists<libra2_coin::MintCapStore>(signer::address_of(core_resources));
        include GetSignerAbortsIf;
    }
    ...
    spec schema GetVotingPowerAbortsIf {
        ...
        aborts_if !allow_validator_set_change && !exists<stake::ValidatorSet>(@libra2_framework);
    ...
    spec schema GetSignerAbortsIf {
        ...
        aborts_if !exists<GovernanceResponsbility>(@libra2_framework);
        let cap_map = global<GovernanceResponsbility>(@libra2_framework).signer_caps;
    ...
    spec assert_proposal_expiration(stake_pool: address, proposal_id: u64) {
        include VotingInitializationAbortIfs;
        include voting::AbortsIfNotContainProposalID<GovernanceProposal>{voting_forum_address: @libra2_framework};
        let proposal_expiration = voting::spec_get_proposal_expiration_secs<GovernanceProposal>(@libra2_framework, proposal_id);
    ...
    spec force_end_epoch(libra2_framework: &signer) {
        use libra2_framework::reconfiguration_with_dkg;
        use std::signer;
        pragma verify = false; // TODO: set because of timeout (property proved).
        let address = signer::address_of(libra2_framework);
        include reconfiguration_with_dkg::FinishRequirement {
            framework: libra2_framework
        };
    }
