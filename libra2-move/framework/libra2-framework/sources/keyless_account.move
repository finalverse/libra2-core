...
module libra2_framework::keyless_account {
    ...
    // The `libra2_framework::reconfiguration_with_dkg` module needs to be able to call `on_new_epoch`.
    ...
    #[resource_group_member(group = libra2_framework::keyless_account::Group)]
    ...
    public(friend) fun on_new_epoch(fx: &signer) acquires Groth16VerificationKey, Configuration {
        ...
        if (config_buffer::does_exist<Groth16VerificationKey>()) {
            ...
            if (exists<Groth16VerificationKey>(@libra2_framework)) {
                *borrow_global_mut<Groth16VerificationKey>(@libra2_framework) = vk;
            } else {
                move_to(fx, vk);
            }
        };
        ...
        if (config_buffer::does_exist<Configuration>()) {
            ...
            if (exists<Configuration>(@libra2_framework)) {
                *borrow_global_mut<Configuration>(@libra2_framework) = config;
            } else {
                move_to(fx, config);
            }
        };
