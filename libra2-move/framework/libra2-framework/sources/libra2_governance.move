...
module libra2_framework::libra2_governance {
    use std::error;
    use std::option;
    use std::signer;
    ...
    use libra2_framework::governance_proposal::{Self, GovernanceProposal};
    ...
    use libra2_framework::reconfiguration_with_dkg;
    use libra2_framework::timestamp;
    use libra2_framework::voting;
    ...
    public fun store_signer_cap(
        ...
        if (!exists<GovernanceResponsbility>(@libra2_framework)) {
            move_to(
                libra2_framework,
        ...
        let signer_caps = &mut borrow_global_mut<GovernanceResponsbility>(@libra2_framework).signer_caps;
    ...
    fun initialize(
        ...
        voting::register<GovernanceProposal>(libra2_framework);
        initialize_partial_voting(libra2_framework);
        move_to(libra2_framework, GovernanceConfig {
            voting_duration_secs,
            min_voting_threshold,
            required_proposer_stake,
        });
        move_to(libra2_framework, GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(libra2_framework),
            update_config_events: account::new_event_handle<UpdateConfigEvent>(libra2_framework),
            vote_events: account::new_event_handle<VoteEvent>(libra2_framework),
        });
        move_to(libra2_framework, VotingRecords {
            votes: table::new(),
        });
        move_to(libra2_framework, ApprovedExecutionHashes {
    ...
    public fun update_governance_config(
        ...
        let governance_config = borrow_global_mut<GovernanceConfig>(@libra2_framework);
        ...
        if (std::features::module_event_migration_enabled()) {
            ...
            let events = borrow_global_mut<GovernanceEvents>(@libra2_framework);
    ...
    public fun initialize_partial_voting(
        ...
        move_to(libra2_framework, VotingRecordsV2 {
    ...
    public fun get_voting_duration_secs(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@libra2_framework).voting_duration_secs
    }

    #[view]
    public fun get_min_voting_threshold(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@libra2_framework).min_voting_threshold
    }

    #[view]
    public fun get_required_proposer_stake(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@libra2_framework).required_proposer_stake
    }
    ...
    public fun has_entirely_voted(stake_pool: address, proposal_id: u64): bool acquires VotingRecords {
        ...
        let voting_records = borrow_global<VotingRecords>(@libra2_framework);
    ...
    public fun get_remaining_voting_power(
        ...
        if (features::partial_governance_voting_enabled()) {
            let voting_records_v2 = borrow_global<VotingRecordsV2>(@libra2_framework);
    ...
    public fun create_proposal_v2_impl(
        ...
        let governance_config = borrow_global<GovernanceConfig>(@libra2_framework);
        ...
        let total_voting_token_supply = coin::supply<Libra2Coin>();
    ...
    fun vote_internal(
        ...
        if (features::partial_governance_voting_enabled()) {
            let voting_records_v2 = borrow_global_mut<VotingRecordsV2>(@libra2_framework);
            ...
            *used_voting_power = *used_voting_power + voting_power;
            ...
            let voting_records = borrow_global_mut<VotingRecords>(@libra2_framework);
            assert!(
                !table::contains(&voting_records.votes, record_key),
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(@libra2_framework, proposal_id);
    ...
    public fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes = borrow_global_mut<ApprovedExecutionHashes>(@libra2_framework);
        ...
        let execution_hash = voting::get_execution_hash<GovernanceProposal>(@libra2_framework, proposal_id);
    ...
    public fun resolve(
        ...
        voting::resolve<GovernanceProposal>(@libra2_framework, proposal_id);
    ...
    public fun resolve_multi_step_proposal(
        ...
        voting::resolve_proposal_v2<GovernanceProposal>(@libra2_framework, proposal_id, next_execution_hash);
    ...
    public fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            voting::is_resolved<GovernanceProposal>(@libra2_framework, proposal_id),
            error::invalid_argument(EPROPOSAL_NOT_RESOLVED_YET),
        );

        let approved_hashes = &mut borrow_global_mut<ApprovedExecutionHashes>(@libra2_framework).hashes;
    ...
    public entry fun reconfigure(libra2_framework: &signer) {
        system_addresses::assert_libra2_framework(libra2_framework);
        if (consensus_config::validator_txn_enabled() && randomness_config::enabled()) {
            reconfiguration_with_dkg::try_start();
        } else {
            reconfiguration_with_dkg::finish(libra2_framework);
        }
    }
    ...
    public entry fun force_end_epoch(libra2_framework: &signer) {
        system_addresses::assert_libra2_framework(libra2_framework);
        reconfiguration_with_dkg::finish(libra2_framework);
    }
    ...
    public entry fun force_end_epoch_test_only(libra2_framework: &signer) acquires GovernanceResponsbility {
        let core_signer = get_signer_testnet_only(libra2_framework, @0x1);
        system_addresses::assert_libra2_framework(&core_signer);
        reconfiguration_with_dkg::finish(&core_signer);
    }

    /// Update feature flags and also trigger reconfiguration.
    public fun toggle_features(libra2_framework: &signer, enable: vector<u64>, disable: vector<u64>) {
        system_addresses::assert_libra2_framework(libra2_framework);
        features::change_feature_flags_for_next_epoch(libra2_framework, enable, disable);
        reconfigure(libra2_framework);
    }
    ...
    public fun get_signer_testnet_only(
        ...
        assert!(libra2_coin::has_mint_capability(core_resources), error::unauthenticated(EUNAUTHORIZED));
        get_signer(signer_address)
    }
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility = borrow_global<GovernanceResponsbility>(@libra2_framework);
    ...
    fun assert_voting_initialization() {
        if (features::partial_governance_voting_enabled()) {
            assert!(exists<VotingRecordsV2>(@libra2_framework), error::invalid_state(EPARTIAL_VOTING_NOT_INITIALIZED));
        };
    }
    ...
    public entry fun test_voting_generic(
        ...
        setup_partial_voting(&libra2_framework, &proposer, &yes_voter, &no_voter);
        ...
        test_resolving_proposal_generic(libra2_framework, use_generic_resolve_function, execution_hash);
    }
    ...
    public entry fun test_resolving_proposal_generic(
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(signer::address_of(&libra2_framework), 0);
        assert!(proposal_state == PROPOSAL_STATE_SUCCEEDED, proposal_state);

        // Add approved script hash.
        add_approved_script_hash(0);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@libra2_framework).hashes;
        assert!(*simple_map::borrow(&approved_hashes, &0) == execution_hash, 0);

        // Resolve the proposal.
        let account = resolve_proposal_for_test(0, @libra2_framework, use_generic_resolve_function, true);
        assert!(signer::address_of(&account) == @libra2_framework, 1);
        assert!(voting::is_resolved<GovernanceProposal>(@libra2_framework, 0), 2);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@libra2_framework).hashes;
        assert!(!simple_map::contains_key(&approved_hashes, &0), 3);
    }

    #[test(libra2_framework = @libra2_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    public entry fun test_voting(
        ...
        test_voting_generic(libra2_framework, proposer, yes_voter, no_voter, false, false);
    }
    ...
    public entry fun test_voting_multi_step(
        ...
        test_voting_generic(libra2_framework, proposer, yes_voter, no_voter, true, true);
    }

    #[test(libra2_framework = @libra2_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    #[expected_failure(abort_code = 0x5000a, location = libra2_framework::voting)]
    public entry fun test_voting_multi_step_cannot_use_single_step_resolve(
        ...
        test_voting_generic(libra2_framework, proposer, yes_voter, no_voter, true, false);
    }
    ...
    public entry fun test_voting_single_step_can_use_generic_resolve_function(
        ...
        test_voting_generic(libra2_framework, proposer, yes_voter, no_voter, false, true);
    }
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_generic(
        ...
        if (multi_step) {
            ...
            vector::push_back(&mut execution_hash, 1);
            voting::resolve_proposal_v2<GovernanceProposal>(@libra2_framework, 0, next_execution_hash);
            assert!(voting::is_resolved<GovernanceProposal>(@libra2_framework, 0), 0);
            if (vector::length(&next_execution_hash) == 0) {
            ...
            voting::resolve<GovernanceProposal>(@libra2_framework, 0);
            assert!(voting::is_resolved<GovernanceProposal>(@libra2_framework, 0), 0);
            remove_approved_hash(0);
        };
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@libra2_framework).hashes;
    ...
    #[test(libra2_framework = @libra2_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 65541, location = libra2_framework::libra2_governance)]
    public entry fun test_cannot_double_vote(
        ...
        setup_voting(&libra2_framework, &proposer, &voter_1, &voter_2);
    ...
    public entry fun test_cannot_vote_for_expired_proposal(
        ...
        setup_partial_voting_with_initialized_stake(&libra2_framework, &proposer, &voter_1, &voter_2);
    ...
    #[test(libra2_framework = @libra2_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @0x345)]
    ...
    public entry fun test_stake_pool_can_vote_on_partial_voting_proposal_many_times(
        ...
        setup_partial_voting(&libra2_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        ...
        test_resolving_proposal_generic(libra2_framework, true, execution_hash);
    }
    ...
    public entry fun test_batch_vote(
        ...
        features::change_feature_flags_for_testing(&libra2_framework, vector[features::get_coin_to_fungible_asset_migration_feature()], vector[]);
        setup_partial_voting(&libra2_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        let voter_1_addr = signer::address_of(&voter_1);
    ...
    public entry fun test_stake_pool_can_vote_before_and_after_partial_governance_voting_enabled(
        ...
        features::change_feature_flags_for_testing(&libra2_framework, vector[], vector[features::get_partial_governance_voting()]);
        setup_voting(&libra2_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        let proposer_addr = signer::address_of(&proposer);
        ...
        features::change_feature_flags_for_testing(&libra2_framework, vector[features::get_partial_governance_voting()], vector[]);

        coin::register<Libra2Coin>(&voter_1);
        coin::register<Libra2Coin>(&voter_2);
    ...
    public fun setup_voting(
        ...
        account::create_account_for_test(signer::address_of(libra2_framework));
        ...
        staking_config::initialize_for_test(libra2_framework, 0, 1000, 2000, true, 0, 1, 100);
        initialize(libra2_framework, 10, 100, 1000);
        store_signer_cap(
            libra2_framework,
            @libra2_framework,
            account::create_test_signer_cap(@libra2_framework),
        );
        ...
        stake::create_validator_set(libra2_framework, active_validators, pks);
        ...
        coin::destroy_mint_cap<Libra2Coin>(mint_cap);
    ...
    public fun setup_voting_with_initialized_stake(
        ...
        // Initialize the governance.
        stake::initialize_for_test_custom(libra2_framework, 0, 1000, 2000, true, 0, 1, 1000);
        initialize(libra2_framework, 10, 100, 1000);
        store_signer_cap(
            libra2_framework,
            @libra2_framework,
    ...
    public fun setup_partial_voting_with_initialized_stake(
        ...
        features::change_feature_flags_for_testing(libra2_framework, vector[features::get_partial_governance_voting()], vector[]);
        setup_voting_with_initialized_stake(libra2_framework, proposer, yes_voter, no_voter);
    }
    ...
    public entry fun test_update_governance_config(
        ...
        account::create_account_for_test(signer::address_of(&libra2_framework));
        initialize(&libra2_framework, 1, 2, 3);
        update_governance_config(&libra2_framework, 10, 20, 30);

        let config = borrow_global<GovernanceConfig>(@libra2_framework);
    ...
    #[test(account = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = libra2_framework::system_addresses)]
    ...
    public fun initialize_for_test(
        ...
        initialize(libra2_framework, min_voting_threshold, required_proposer_stake, voting_duration_secs);
    }
