...
module libra2_framework::aptos_coin {
    use std::error;
    use std::signer;
    use std::string;
    use std::vector;
    ...
    public(friend) fun initialize(libra2_framework: &signer): (BurnCapability<AptosCoin>, MintCapability<AptosCoin>) {
    ...
    #[test_only]
    public fun initialize_for_test(libra2_framework: &signer): (BurnCapability<AptosCoin>, MintCapability<AptosCoin>) {
        aggregator_factory::initialize_aggregator_factory_for_test(libra2_framework);
        let (burn_cap, mint_cap) = initialize(libra2_framework);
        coin::create_coin_conversion_map(libra2_framework);
        coin::create_pairing<AptosCoin>(libra2_framework);
