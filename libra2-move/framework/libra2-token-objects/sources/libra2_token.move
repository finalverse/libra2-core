...
module libra2_token_objects::libra2_token {
    use std::error;
    ...
    struct Libra2Collection has key {
    ...
    public fun mint_token_object(
        ...
        property_types: vector<String>,
        property_values: vector<vector<u8>>,
    ): Object<Libra2Token> acquires Libra2Collection, Libra2Token {
        ...
        if (freezable_by_creator) {
            let libra2_token_addr = object::address_from_constructor_ref(&constructor_ref);
            let libra2_token = borrow_global_mut<Libra2Token>(libra2_token_addr);
            let transfer_ref = object::generate_transfer_ref(&constructor_ref);
            option::fill(&mut libra2_token.transfer_ref, transfer_ref);
        };
    ...
    public fun mint_soul_bound_token_object(
        ...
        property_values: vector<vector<u8>>,
        soul_bound_to: address,
    ): Object<Libra2Token> acquires Libra2Collection {
        let constructor_ref = mint_internal(
            creator,
            collection,
            description,
    ...
    fun mint_internal(
        ...
        property_types: vector<String>,
        property_values: vector<vector<u8>>,
    ): ConstructorRef acquires Libra2Collection {
    ...
    inline fun borrow<T: key>(token: &Object<T>): &Libra2Token {
        let token_address = object::object_address(token);
        assert!(
            exists<Libra2Token>(token_address),
            error::not_found(ETOKEN_DOES_NOT_EXIST),
        );
        borrow_global<Libra2Token>(token_address)
    }

    #[view]
    public fun are_properties_mutable<T: key>(token: Object<T>): bool acquires Libra2Collection {
        let collection = token::collection_object(token);
        borrow_collection(&collection).mutable_token_properties
    }

    #[view]
    public fun is_burnable<T: key>(token: Object<T>): bool acquires Libra2Token {
        option::is_some(&borrow(&token).burn_ref)
    }

    #[view]
    public fun is_freezable_by_creator<T: key>(token: Object<T>): bool acquires Libra2Collection {
        are_collection_tokens_freezable(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_description<T: key>(token: Object<T>): bool acquires Libra2Collection {
        is_mutable_collection_token_description(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_name<T: key>(token: Object<T>): bool acquires Libra2Collection {
        is_mutable_collection_token_name(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_uri<T: key>(token: Object<T>): bool acquires Libra2Collection {
        is_mutable_collection_token_uri(token::collection_object(token))
    }

    // Token mutators

    inline fun authorized_borrow<T: key>(token: &Object<T>, creator: &signer): &Libra2Token {
        let token_address = object::object_address(token);
        assert!(
            exists<Libra2Token>(token_address),
            error::not_found(ETOKEN_DOES_NOT_EXIST),
    ...
    public entry fun burn<T: key>(creator: &signer, token: Object<T>) acquires Libra2Token {
        let libra2_token = authorized_borrow(&token, creator);
        assert!(
            option::is_some(&libra2_token.burn_ref),
            error::permission_denied(ETOKEN_NOT_BURNABLE),
        );
        move libra2_token;
        let libra2_token = move_from<Libra2Token>(object::object_address(&token));
        let Libra2Token {
            burn_ref,
            transfer_ref: _,
            mutator_ref: _,
    ...
    public entry fun freeze_transfer<T: key>(creator: &signer, token: Object<T>) acquires Libra2Collection, Libra2Token {
        let libra2_token = authorized_borrow(&token, creator);
        assert!(
            are_collection_tokens_freezable(token::collection_object(token))
                && option::is_some(&libra2_token.transfer_ref),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        object::disable_ungated_transfer(option::borrow(&libra2_token.transfer_ref));
    }
    ...
    public entry fun unfreeze_transfer<T: key>(
        ...
        object::enable_ungated_transfer(option::borrow(&libra2_token.transfer_ref));
    }
    ...
    public entry fun set_description<T: key>(
        ...
        let libra2_token = authorized_borrow(&token, creator);
        token::set_description(option::borrow(&libra2_token.mutator_ref), description);
    }
    ...
    public entry fun set_name<T: key>(
        ...
        let libra2_token = authorized_borrow(&token, creator);
        token::set_name(option::borrow(&libra2_token.mutator_ref), name);
    }
    ...
    public entry fun set_uri<T: key>(
        ...
        let libra2_token = authorized_borrow(&token, creator);
        token::set_uri(option::borrow(&libra2_token.mutator_ref), uri);
    }
    ...
    public entry fun add_property<T: key>(
        ...
        property_map::add(&libra2_token.property_mutator_ref, key, type, value);
    }
    ...
    public entry fun add_typed_property<T: key, V: drop>(
        ...
        property_map::add_typed(&libra2_token.property_mutator_ref, key, value);
    }
    ...
    public entry fun remove_property<T: key>(
        ...
        property_map::remove(&libra2_token.property_mutator_ref, &key);
    }
    ...
    public entry fun update_property<T: key>(
        ...
        property_map::update(&libra2_token.property_mutator_ref, &key, type, value);
    }
    ...
    public entry fun update_typed_property<T: key, V: drop>(
        ...
        property_map::update_typed(&libra2_token.property_mutator_ref, &key, value);
    }

    // Collection accessors

    inline fun collection_object(creator: &signer, name: &String): Object<Libra2Collection> {
        let collection_addr = collection::create_collection_address(&signer::address_of(creator), name);
        object::address_to_object<Libra2Collection>(collection_addr)
    }

    inline fun borrow_collection<T: key>(token: &Object<T>): &Libra2Collection {
        let collection_address = object::object_address(token);
        assert!(
            exists<Libra2Collection>(collection_address),
            error::not_found(ECOLLECTION_DOES_NOT_EXIST),
        );
        borrow_global<Libra2Collection>(collection_address)
    }

    public fun is_mutable_collection_description<T: key>(
        collection: Object<T>,
    ): bool acquires Libra2Collection {
    ...
    inline fun authorized_borrow_collection<T: key>(collection: &Object<T>, creator: &signer): &Libra2Collection {
    ...
    public entry fun set_collection_description<T: key>(
        ...
        collection::set_description(option::borrow(&libra2_collection.mutator_ref), description);
    }
    ...
    public fun set_collection_royalties<T: key>(
        ...
        let libra2_collection = authorized_borrow_collection(&collection, creator);
        assert!(
            option::is_some(&libra2_collection.royalty_mutator_ref),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        royalty::update(option::borrow(&libra2_collection.royalty_mutator_ref), royalty);
    }
    ...
    public entry fun set_collection_uri<T: key>(
        ...
        collection::set_uri(option::borrow(&libra2_collection.mutator_ref), uri);
    }
    ...
    #[test(creator = @0x123)]
    fun test_create_and_transfer(creator: &signer) acquires Libra2Collection, Libra2Token {
    ...
    fun test_mint_soul_bound(creator: &signer, bob: &signer) acquires Libra2Collection {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = object)]
    fun test_frozen_transfer(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_unfrozen_transfer(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    fun test_noncreator_freeze(creator: &signer, another: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123, another = @0x456)]
    #[expected_failure(abort_code = 0x50003, location = Self)]
    fun test_noncreator_unfreeze(creator: &signer, another: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_description(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    fun test_set_immutable_description(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_name(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_name(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_uri(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_uri(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_burnable(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        ...
        assert!(exists<Libra2Token>(token_addr), 0);
        burn(creator, token);
        assert!(!exists<Libra2Token>(token_addr), 1);
    }

    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50005, location = Self)]
    fun test_not_burnable(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_collection_description(creator: &signer) acquires Libra2Collection {
        let collection_name = string::utf8(b"collection name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_collection_description(creator: &signer) acquires Libra2Collection {
        let collection_name = string::utf8(b"collection name");
    ...
    #[test(creator = @0x123)]
    fun test_set_collection_uri(creator: &signer) acquires Libra2Collection {
        let collection_name = string::utf8(b"collection name");
        let collection = create_collection_helper(creator, collection_name, true);
        let value = string::utf8(b"not");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_collection_uri(creator: &signer) acquires Libra2Collection {
        let collection_name = string::utf8(b"collection name");
        let collection = create_collection_helper(creator, collection_name, false);
    ...
    #[test(creator = @0x123)]
    fun test_property_add(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_property_typed_add(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"u8");
        ...
        create_collection_helper(creator, collection_name, true);
        let token = mint_helper(creator, collection_name, token_name);
        add_typed_property<Libra2Token, u8>(creator, token, property_name, 0x8);

        assert!(property_map::read_u8(&token, &property_name) == 0x8, 0);
    }

    #[test(creator = @0x123)]
    fun test_property_update(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_property_update_typed(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"bool");
        ...
        create_collection_helper(creator, collection_name, true);
        let token = mint_helper(creator, collection_name, token_name);
        update_typed_property<Libra2Token, bool>(creator, token, property_name, false);

        assert!(!property_map::read_bool(&token, &property_name), 0);
    }

    #[test(creator = @0x123)]
    fun test_property_remove(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"bool");
    ...
    #[test(creator = @0x123)]
    fun test_royalties(creator: &signer) acquires Libra2Collection, Libra2Token {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
