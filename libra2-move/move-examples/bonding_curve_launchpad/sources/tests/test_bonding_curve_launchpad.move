...
module bonding_curve_launchpad::test_bonding_curve_launchpad {
    ...
    fun test_setup_accounts(
        libra2_framework: &signer, _swap_dex_signer: &signer, bonding_curve_creator: &signer
    ) {
    ...
    #[test(deployer = @bonding_curve_launchpad)]
    #[expected_failure(abort_code = 393218, location = libra2_framework::object)]
    ...
    fun test_e2e_bonding_curve_creation(
        ...
        test_setup_accounts(libra2_framework, swap_dex_signer, bonding_curve_creator);
    ...
    fun test_e2e_directional_swaps(
        ...
        test_e2e_bonding_curve_creation(libra2_framework, swap_dex_signer, bcl_owner_signer, bonding_curve_creator);
        let user_address = signer::address_of(bonding_curve_creator);
        let name = string::utf8(b"SheepyCoin");
        let symbol = string::utf8(b"SHEEP");
        let starting_apt_balance = coin::balance<AptosCoin>(user_address);
    ...
    fun test_e2e_swap_after_graduation(
        ...
        test_e2e_graduation(libra2_framework, swap_dex_signer, bcl_owner_signer, bonding_curve_creator);
        let fa_obj_metadata =
