...
You also need a recent version of the [Aptos CLI](https://dev.libra2.org/tools/aptos-cli/install-cli/). This tutorial is written using the following version:
...
* Move code can also live in a number of other places. See the [Move
  book](https://dev.libra2.org/move/book/packages) for more information on the
  Move package system.
* More information on the `Move.toml` file can also be found in the [Package](https://dev.libra2.org/move/book/packages#movetoml) section of the Move book.
* Move also supports the idea of [named
  addresses](https://dev.libra2.org/move/book/address#named-addresses); Named
  ...
  that you can deploy, depending on what address(es) you control. They are used quite frequently, and can be defined in the `Move.toml` file in the `[addresses]` section, like so:
    ...
    ```
* [Structures](https://dev.libra2.org/move/book/structs-and-resources) in Move
  can be given different
  [abilities](https://dev.libra2.org/move/book/abilities) that describe what
...
* [Functions](https://dev.libra2.org/move/book/functions) are default
    private, and can also be `public`,
    [`public(friend)`](https://dev.libra2.org/move/book/friends).
    A function marked as `entry` can be called as a transaction.
* `move_to` is one of the [five different global storage
  operators](https://dev.libra2.org/move/book/global-storage-operators).
</details>
...
* There are a number of [test-related annotations](https://dev.libra2.org/move/book/unit-testing#testing-annotations-their-meaning-and-usage) that are worth exploring.
...
```
Read more on Move function [visibilities](https://dev.libra2.org/move/book/functions#visibility).
</details>
<details>
...
if an [abort](https://dev.libra2.org/move/book/abort-and-assert) is raised no unwinding of state
needs to be performed, as no changes from that transaction will be persisted to the blockchain.

[`error` module]: https://github.com/finalverse/libra2-core/blob/main/libra2-move/framework/move-stdlib/sources/error.move
...
to get a mutable reference to the global storage, and `&mut` is used to create a mutable [reference](https://dev.libra2.org/move/book/references) to a field of a
...
Read more about [phantom type](https://dev.libra2.org/move/book/generics#phantom-type-parameters) parameters in the Aptos Move Book.
</details>
