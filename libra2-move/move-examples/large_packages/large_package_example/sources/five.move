...
///
///
/// At creation, an [Aptos account](https://github.com/finalverse/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/account.move#L23) contains:
/// * A [resource containing Aptos Coin](https://github.com/finalverse/libra2-core/blob/60751b5ed44984178c7163933da3d1b18ad80388/libra2-move/framework/libra2-framework/sources/coin.move#L50) and deposit and withdrawal of coins from that resource.
...
/// - A script (payload)
///
/// Both [Python](https://github.com/finalverse/libra2-core/blob/3973311dac6bb9348bfc81cf983c2a1be11f1b48/ecosystem/python/sdk/libra2_sdk/client.py#L256) and [TypeScript](https://github.com/finalverse/libra2-core/blob/3973311dac6bb9348bfc81cf983c2a1be11f1b48/ecosystem/typescript/sdk/src/libra2_client.test.ts#L93) support the generation of transactions that target entry points. This guide points out many of those entry points, such as `libra2_account::transfer` and `libra2_account::create_account`.
///
/// Most basic operations on the Aptos blockchain should be available via entry point calls. While one could submit multiple transactions calling entry points in series, such operations benefit from being called atomically from a single transaction. A script payload transaction can call any public (entry) function defined within any module. Here's an example [Move script](https://github.com/finalverse/libra2-core/tree/main/libra2-move/move-examples/scripts/two_by_two_transfer) that uses a MultiAgent transaction to extract funds from two accounts and deposit them into two other accounts. This is a [Python example](https://github.com/finalverse/libra2-core/blob/main/ecosystem/python/sdk/examples/transfer-two-by-two.py) that uses the bytecode generated by compiling that script. Currently there is limited support for script payloads in TypeScript.
///
...
///
///
/// Both the [Typescript SDK](https://github.com/finalverse/libra2-core/blob/9b85d41ed8ef4a61a9cd64f9de511654fcc02024/ecosystem/typescript/sdk/src/libra2_client.ts#L413) and [Python SDK](https://github.com/finalverse/libra2-core/blob/main/ecosystem/python/sdk/examples/simulate-transfer-coin.py) support the simulation API. Note the output and gas used may change based upon the state of the account. For gas estimations, we recommend that the maximum gas amount be larger than the amount quoted by this API.
///
...
///
/// Events and transactions pruning can be disabled via setting the [`enable_ledger_pruner`](https://github.com/finalverse/libra2-core/blob/cf0bc2e4031a843cdc0c04e70b3f7cd92666afcf/config/src/config/storage_config.rs#L141) to `false`. This is default behavior in Mainnet. In the near future, Aptos will provide indexers that mitigate the need to directly query from a node.
///
...
///
/// Aptos has a standard [Coin type](https://github.com/finalverse/libra2-core/blob/main/libra2-move/framework/libra2-framework/sources/coin.move). Different types of coins can be represented in this type through the use of distinct structs that represent the type parameter or generic for `Coin<T>`.
///
...
///
/// Coins can be transferred between users via the [`coin::transfer`](https://github.com/finalverse/libra2-core/blob/36a7c00b29a457469264187d8e44070b2d5391fe/libra2-move/framework/libra2-framework/sources/coin.move#L307) function for all coins and [`libra2_account::transfer`](https://github.com/finalverse/libra2-core/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/libra2-move/framework/libra2-framework/sources/libra2_account.move#L18) for Aptos coins. The advantage of the latter function is that it creates the destination account if it does not exist.
///
/// :::caution
...
/// ```
/// {
///   "type": "0x1::coin::CoinStore<0x1::libra2_coin::Libra2Coin>",
///   "data": {
///     "coin": {
///       "value": "3927"
///     },
...
/// 3. A gas fee `2200` of `0x1::libra2_coin::Libra2Coin` from the sending account `0x810026ca8291dd88b5b30a1d3ca2edd683d33d06c4a7f7c451d96f6d47bc5e8b`
///
...
/// 2. Gas is always: `0x1::libra2_coin::Libra2Coin`
///
...
/// 1. Generate a client from the [OpenAPI spec](https://github.com/finalverse/libra2-core/blob/main/crates/libra2-faucet/doc/spec.yaml).
