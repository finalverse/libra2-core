...
impl AptosModuleCacheManager {
    ...
    pub fn try_lock(
            ...
            && config.prefetch_framework_code
        {
            ...
            prefetch_libra2_framework(code_storage, guard.module_cache_mut()).map_err(|err| {
...
mod test {
    ...
    #[test]
    fn test_prefetch_existing_libra2_framework() {
        ...
        let result = prefetch_libra2_framework(code_storage, &mut module_cache);
        assert!(result.is_ok());
        assert!(module_cache.num_modules() > 0);
    }

    #[test]
    fn test_prefetch_non_existing_libra2_framework() {
