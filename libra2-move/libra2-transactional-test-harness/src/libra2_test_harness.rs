...
fn libra2_framework_private_key_mapping() -> Vec<(String, Ed25519PrivateKey)> {
...
static PRECOMPILED_APTOS_FRAMEWORK_V1: Lazy<Option<(FullyCompiledProgram, Vec<PackagePaths>)>> =
    Lazy::new(|| {
        ...
        let program_res = move_compiler::construct_pre_compiled_lib(
            ...
            libra2_framework::extended_checks::get_all_attribute_names(),
        )
        .unwrap();
        match program_res {
            Ok(af) => Some((af, vec![lib_paths_movesym])),
            Err((files, errors)) => {
                eprintln!("!!!Libra2 Framework failed to compile!!!");
...
static PRECOMPILED_APTOS_FRAMEWORK_V2: Lazy<PrecompiledFilesModules> = Lazy::new(|| {
    ...
    let options = move_compiler_v2::Options {
        ...
        known_attributes: libra2_framework::extended_checks::get_all_attribute_names().clone(),
...
impl<'a> MoveTestAdapter<'a> for AptosTestAdapter<'a> {
    ...
    fn init(
        ...
        let mut named_address_mapping = libra2_framework::named_addresses().clone();
        ...
        for (name, private_key) in libra2_framework_private_key_mapping() {
        ...
        let warnings_opt = match (syntax, opt_model) {
            ...
            (_, Some(model)) => {
                let _runtime_metadata =
                    libra2_framework::extended_checks::run_extended_checks(&model);
