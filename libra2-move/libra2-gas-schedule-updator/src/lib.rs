...
fn generate_script(gas_schedule: &GasScheduleV2) -> Result<String> {
    ...
    emitln!(writer, "use libra2_framework::aptos_governance;");
    emitln!(writer, "use libra2_framework::gas_schedule;");
    emitln!(writer);
...
fn libra2_framework_path() -> PathBuf {
    Path::join(
...
pub fn generate_update_proposal(args: &GenArgs) -> Result<()> {
    ...
    pack.add_local_dep("Libra2Framework", &libra2_framework_path().to_string_lossy());
