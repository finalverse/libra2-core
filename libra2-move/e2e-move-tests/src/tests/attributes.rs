...
fn test_bad_fun_attribute_in_compiled_module() {
    ...
    let mut value = libra2_framework::RuntimeModuleMetadataV1 {
    ...
    .unwrap();
    let known_attribute =
        bcs::from_bytes::<libra2_framework::KnownAttribute>(&fake_attribute).unwrap();
    value
        .fun_attributes
        .insert("view".to_string(), vec![known_attribute]);

    let metadata = Metadata {
        key: libra2_framework::APTOS_METADATA_KEY_V1.to_vec(),
