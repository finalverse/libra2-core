...
use libra2_language_e2e_tests::account::TransactionBuilder;
use libra2_types::{
...
fn test_script_with_object_parameter() {
    ...
    let root = h.libra2_framework_account();
...
fn test_script_with_type_parameter() {
    ...
    let package = build_package(
        ...
        libra2_framework::BuildOptions::default(),
    )
    .expect("building package must succeed");
...
fn test_two_to_two_transfer() {
    ...
    let build_options = libra2_framework::BuildOptions {
        with_srcs: false,
        with_abis: false,
