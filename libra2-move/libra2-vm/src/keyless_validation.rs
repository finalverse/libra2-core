...
use crate::{libra2_vm::fetch_module_metadata_for_struct_tag, move_vm_ext::AptosMoveResolver};
...
pub(crate) fn validate_authenticators(
    ...
    if authenticators.len() > config.max_signatures_per_txn as usize {
        // println!("[libra2-vm][groth16] Too many keyless authenticators");
    ...
    for (_, sig) in authenticators {
        sig.verify_expiry(onchain_timestamp_obj.microseconds)
            .map_err(|_| {
                // println!("[libra2-vm][groth16] ZKP expired");
