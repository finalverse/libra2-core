...
impl Libra2VM {
    ...
    pub fn new(env: Libra2Environment, state_view: &impl StateView) -> Self {
        let _timer = TIMER.timer_with(&["Libra2VM::new"]);
    ...
    pub(crate) fn failed_transaction_cleanup(
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[libra2-vm][gas-meter][failure-epilogue] {}",
                    err.message()
    ...
    fn finish_aborted_transaction(
        ...
        let (previous_session_change_set, fee_statement) =
            if is_account_init_for_sponsored_transaction {
                ...
                let fee_statement =
                    Libra2VM::fee_statement_from_gas_meter(txn_data, gas_meter, ZERO_STORAGE_REFUND);
    ...
    fn success_transaction_cleanup(
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[libra2-vm][gas-meter][success-epilogue] {}",
                    err.message()
                        .unwrap_or("No message found -- this should not happen.")
                );
        ...
        let fee_statement = Libra2VM::fee_statement_from_gas_meter(
            txn_data,
            gas_meter,
    ...
    fn execute_script_or_entry_function<'a, 'r, 'l>(
        ...
        fail_point!("libra2_vm::execute_script_or_entry_function", |_| {
            Err(VMStatus::Error {
    ...
    fn validate_publish_request(
        ...
        for m in modules {
            ...
            libra2_framework::verify_module_metadata(m, self.features(), self.timed_features())
        ...
        if self.chain_id().is_mainnet() {
            for module in modules {
                if let Some(metadata) =
                    libra2_framework::get_compilation_metadata_from_compiled_module(module)
                {
    ...
    pub fn reject_unstable_bytecode_for_script(&self, module: &CompiledScript) -> VMResult<()> {
        if self.chain_id().is_mainnet() {
            if let Some(metadata) =
                libra2_framework::get_compilation_metadata_from_compiled_script(module)
            {
                if metadata.unstable {
                    return Err(PartialVMError::new(StatusCode::UNSTABLE_BYTECODE_REJECTED)
    ...
    fn execute_user_transaction_impl(
        ...
        let account_init_for_sponsored_transaction_timer =
            VM_TIMER.timer_with_label("Libra2VM::account_init_for_sponsored_transaction");
        ...
        let payload_timer =
            VM_TIMER.timer_with_label("Libra2VM::execute_user_transaction_impl [payload]");
        ...
        match write_set_payload {
            WriteSetPayload::Direct(change_set) => {
                ...
                if !Self::should_restart_execution(change_set.events()) {
                    ...
                    error!(
                        "[libra2_vm] direct write set finished without requiring should_restart_execution");
                }
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[libra2_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    fn extract_module_metadata(
        ...
        if self.features().is_enabled(FeatureFlag::VM_BINARY_FORMAT_V6) {
            libra2_framework::get_vm_metadata(&self.move_vm, module_storage, module_id)
        } else {
            libra2_framework::get_vm_metadata_v0(&self.move_vm, module_storage, module_id)
        }
    }
    ...
    pub fn execute_single_transaction(
        ...
        Ok(match txn.expect_valid() {
            Transaction::BlockMetadata(block_metadata) => {
                fail_point!("libra2_vm::execution::block_metadata");
            ...
            Transaction::BlockMetadataExt(block_metadata_ext) => {
                fail_point!("libra2_vm::execution::block_metadata_ext");
            ...
            Transaction::UserTransaction(txn) => {
                fail_point!("libra2_vm::execution::user_transaction");
                    ...
                    match vm_status.status_code() {
                        ...
                        | StatusCode::TYPE_RESOLUTION_FAILURE => {
                            speculative_error!(
                                log_context,
                                format!(
                                    "[libra2_vm] Transaction breaking invariant violation. txn: {:?}, status: {:?}",
                        ...
                        if vm_status.sub_status()
                            == Some(move_core_types::vm_status::sub_status::unknown_invariant_violation::EPARANOID_FAILURE) =>
                            {
                                error!(
                                *log_context,
                                "[libra2_vm] Transaction breaking paranoid mode. txn: {:?}, status: {:?}",
                                bcs::to_bytes::<SignedTransaction>(txn),
                                vm_status,
                            );
                            },
...
pub struct Libra2VMBlockExecutor {
...
impl Libra2VMBlockExecutor {
    ...
    pub fn execute_block_with_config(
        ...
        fail_point!("libra2_vm_block_executor::execute_block_with_config", |_| {
            Err(VMStatus::error(
                StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR,
                None,
            ))
        ...
        let result = Libra2VMBlockExecutorWrapper::execute_block::<
            _,
...
impl VMBlockExecutor for Libra2VMBlockExecutor {
    fn new() -> Self {
        Self {
    ...
    fn execute_block(
        ...
        let config = BlockExecutorConfig {
            local: BlockExecutorLocalConfig {
                concurrency_level: Libra2VM::get_concurrency_level(),
                allow_fallback: true,
                discard_failed_blocks: Libra2VM::get_discard_failed_blocks(),
    ...
    fn execute_block_sharded<S: StateView + Sync + Send + 'static, C: ExecutorClient<S>>(
        ...
        let ret = sharded_block_executor.execute_block(
            state_view,
            transactions,
            Libra2VM::get_concurrency_level(),
            onchain_config,
        );
        if ret.is_ok() {
            // Record the histogram count for transactions per block.
...
// Ensure encapsulation of Libra2VM APIs by using a wrapper.
pub struct AptosSimulationVM(Libra2VM);

impl AptosSimulationVM {
    pub fn new(env: Libra2Environment, state_view: &impl StateView) -> Self {
        let mut vm = Libra2VM::new(env, state_view);
...
#[cfg(test)]
mod tests {
    use crate::{move_vm_ext::MoveVmExt, Libra2VM};
    use libra2_types::{
        account_address::AccountAddress,
    ...
    fn vm_thread_safe() {
        ...
        assert_send::<Libra2VM>();
        assert_sync::<Libra2VM>();
    ...
    fn should_restart_execution_on_new_epoch() {
        ...
        assert!(Libra2VM::should_restart_execution(&[(
            new_epoch_event,
            None
        )]));
        assert!(Libra2VM::should_restart_execution(&[(
