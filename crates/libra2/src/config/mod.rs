...
impl CliCommand<()> for GenerateShellCompletions {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        generate_cli_completions::<Tool>("libra2", self.shell, self.output_file.as_path())
...
pub struct SetGlobalConfig {
    ...
    ///
    /// `Workspace` will put the `.libra2/` folder in the current directory, where
    /// `Global` will put the `.libra2/` folder in your home directory
    #[clap(long)]
...
pub enum ConfigType {
    /// Per system user configuration put in `<HOME>/.libra2`
    Global,
    /// Per directory configuration put in `<CURRENT_DIR>/.libra2`
    Workspace,
}
