...
pub enum CliError {
    ...
    #[error("Unable to find config {0}, have you run `libra2 init`?")]
...
/// Config saved to `.libra2/config.yaml`
...
pub const CONFIG_FOLDER: &str = ".libra2";

/// An individual profile
#[derive(Debug, Default, Serialize, Deserialize)]
pub struct ProfileConfig {
    /// Name of network being used, if setup from libra2 init
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl CliConfig {
    ...
    pub fn config_exists(mode: ConfigSearchMode) -> bool {
        if let Ok(folder) = Self::libra2_folder(mode) {
    ...
    pub fn load(mode: ConfigSearchMode) -> CliTypedResult<Self> {
        let folder = Self::libra2_folder(mode)?;
    ...
    /// Saves the config to ./.libra2/config.yaml
    pub fn save(&self) -> CliTypedResult<()> {
        let libra2_folder = Self::libra2_folder(ConfigSearchMode::CurrentDir)?;

        // Create if it doesn't exist
        let no_dir = !libra2_folder.exists();
        create_dir_if_not_exist(libra2_folder.as_path())?;

        // If the `.libra2/` doesn't exist, we'll add a .gitignore in it to ignore the config file
        ...
        let config_file = libra2_folder.join(CONFIG_FILE);
        ...
        let legacy_config_file = libra2_folder.join(LEGACY_CONFIG_FILE);
    ...
    /// Finds the current directory's .libra2 folder
    fn libra2_folder(mode: ConfigSearchMode) -> CliTypedResult<PathBuf> {
...
impl RestOptions {
    ...
    pub fn url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|p| p.rest_url)
        {
            ...
            Err(CliError::CommandArgumentError("No rest url given.  Please add --url or add a rest_url to the .libra2/config.yaml for the current profile".to_string()))
        }
    }
...
pub struct MovePackageDir {
    ...
    /// See <https://github.com/finalverse/libra2-core/issues/10335>
...
pub fn load_account_arg(str: &str) -> Result<AccountAddress, CliError> {
            ...
            .map(|p| p.private_key)
    {
        ...
        Err(CliError::CommandArgumentError(
            "'--account' or '--profile' after using libra2 init must be provided".to_string(),
        ))
    }
}
...
pub struct FaucetOptions {
    /// URL for the faucet endpoint e.g. `https://faucet.devnet.libra2.org`
    #[clap(long)]
        ...
        match profile.faucet_url {
            ...
            None => match profile.network {
                Some(Network::Mainnet) => {
                    Err(CliError::CommandArgumentError("There is no faucet for mainnet. Please create and fund the account by transferring funds from another account. If you are confident you want to use a faucet, set --faucet-url or add a faucet URL to .libra2/config.yaml for the current profile".to_string()))
                },
                Some(Network::Testnet) => {
                    Err(CliError::CommandArgumentError(format!("To get testnet APT you must visit {}. If you are confident you want to use a faucet programmatically, set --faucet-url or add a faucet URL to .libra2/config.yaml for the current profile", get_mint_site_url(None))))
                },
                _ => {
                    Err(CliError::CommandArgumentError("No faucet given. Please set --faucet-url or add a faucet URL to .libra2/config.yaml for the current profile".to_string()))
                },
            },
        }
    }
